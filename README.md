...
- Python 3.6+
- `requests` library
- `openpyxl` library (for Excel combiner only)
...
### engenius_device_license_export.py

Exports all devices (APs, switches, switch extenders) with license expiration and online/offline status, split into per-state CSVs. Use `--debug` for verbose API output.
...
**Output:** `engenius_export_TIMESTAMP/`

```
Alabama - Wireless APs.csv
Alabama - Switches.csv
California - Wireless APs.csv
California - Switches.csv
...
```
...
### combine_csvs_to_xlsx.py

Combines CSVs from an export folder into a single Excel workbook. Each CSV becomes a tab (e.g. `AL - APs`, `AL - Switches`). Includes header formatting, auto-filter, frozen headers, and color-coded license status and online/offline.

```bash
python3 combine_csvs_to_xlsx.py engenius_export_TIMESTAMP
```

**Output:** `EnGenius_Device_Report_TIMESTAMP.xlsx`
